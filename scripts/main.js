...
function initSplashScreen() {
  ...
  const textParts = ["#splash-text-part1", "#splash-text-part2", "#splash-text-part3"];
  ...
  const sketch = (p) => {
    ...
    p.draw = () => {
      ...
      if (scrollRect.right < p.windowWidth && scrollLeftVal < -50 && !isComplete) {
        isComplete = true;
        autoMode = false;
        ...
        gsap
          ...
          .from(".hero-provider", { opacity: 0, duration: 1, ease: "power3.out" }, "-=0.5");
      }
    };
...
function initBaseEffects() {
  ...
  gsap.utils.toArray(".header-anim").forEach((header) => {
    const line = header.parentElement ? header.parentElement.querySelector(".line-expand") : null;
...
function initThemeAnimations() {
  ...
  if (headerTitle) {
    gsap.set(headerTitle, { clipPath: "inset(100% 0 0 0)", opacity: 0, y: 40 });
  ...
  const headerLine = document.querySelector(".theme-header-section .line-expand");
  if (headerLine) {
...
function initFormatAnimations() {
  ...
  if (speakerHeaderTitle) {
    gsap.set(speakerHeaderTitle, { clipPath: "inset(100% 0 0 0)", opacity: 0, y: 40 });
  ...
  const speakerHeaderLine = document.querySelector(".speaker-header-section .line-expand");
...
function initProjectsAnimations() {
  ...
  if (projectsTitle) {
    gsap.set(projectsTitle, { clipPath: "inset(100% 0 0 0)", opacity: 0, y: 40 });
...
function initEventAnimations() {
  ...
  const headerLine = document.querySelector(".events-header-section .line-expand");
  if (headerLine) {
    const lineTween = gsap.fromTo(
      headerLine,
      { width: 0 },
...
function initRegisterAnimations() {
  ...
  const headerLine = document.querySelector(".register-header-section .line-expand");
  if (headerLine) {
    const lineTween = gsap.fromTo(
      headerLine,
      { width: 0 },
...
function initAboutAnimations() {
  ...
  if (title) {
    gsap.set(title, { clipPath: "inset(100% 0 0 0)", opacity: 0, y: 40, scale: 0.95 });
    const titleTween = gsap.to(title, {
      clipPath: "inset(0% 0 0 0)",
      opacity: 1,
      y: 0,
...
function initProjectsSlider() {
  ...
  const animateSlide = () => {
    gsap.fromTo(
      ...
      { scale: 1, opacity: 1, duration: 0.5, stagger: 0.05, ease: "power2.out", overwrite: "auto" }
    );
  };

  btnNext.addEventListener("click", () => {
    const itemWidth = projectSlider.offsetWidth / (window.innerWidth >= 768 ? 3 : 1);
